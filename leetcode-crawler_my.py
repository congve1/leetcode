...
# usage: python leetcode-crawler_my.py leetcode -u congve1@live.com -p 654852123a -c code
import sqlite3
import json
import traceback
import html2text
...
db_path = 'leetcode.db'
user_agent = r'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/75.0.3761.0 Safari/537.36 Edg/75.0.127.0'
            ...
            try:
                ...
                if content['data']['question']['content'] != None:
                    ...
                    question_detail = (questionId, 
                                ...
                                content['data']['question']['titleSlug'],
                                ...
                                code_snippets)
                    threadLock.acquire()
                    cursor.execute('INSERT INTO question (id, frontend_id, title, slug, difficulty, content, status, titlecn, contentcn, codesnippets) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)', question_detail)
                    for tag in tags:
...
class LeetcodeCrawler():
    ...
    def generate_questions_code_snippets(self, path, filters):
        if not os.path.isdir(path):
            os.mkdir(path)
        cursor = self.conn.cursor()
        cursor.execute("SELECT * FROM question")
        for row in cursor:
            ...
            self.generate_question_code_snippets(question_detail, path)
        cursor.close()
    
    def generate_question_code_snippets(self, question, path):
        id_slug = "{:0>3d}-{}".format(question['frontedId'], question['slug'])
        text_path = os.path.join(path, id_slug, id_slug+".py")
        with open(text_path, 'w', encoding='utf-8') as f:
            f.write(question['codesnippets'])
        print("Generate code snnipets: {} Success".format(text_path))
    ...
    def generate_question_markdown(self, question, path, has_get_code):    
        ...
        with open(os.path.join(text_path, "README.md"), 'w', encoding='utf-8') as f:
            f.write("# {}\n".format(question['titlecn']))
            f.write("\n## 描述\n\n")
            text_cn = question['contentcn']
            content_cn = h.handle(text_cn)
            f.write(content_cn)
            f.write("\n\n")
            ...
            f.write("\n**思路:**\n")
        print("Generate question markdown: {}/README.md Success".format(text_path))
    ...
    def generate_question_submission(self, path, submission):  
        if not os.path.isdir(path):
            os.mkdir(path)
        id_slug = "{:0>3d}-{}".format(submission[3], submission[0])
        text_path = os.path.join(path, id_slug)
        
        if not os.path.isdir(text_path):
            os.mkdir(text_path)   
        with open(os.path.join(text_path, id_slug+".py"), 'w', encoding='utf-8') as f:
            f.write(submission[1].encode('utf-8').decode('unicode_escape'))
        print("Generate solution: {}\{}.py Success".format(text_path, id_slug))
    
    def generate_markdown(self, path, filters):
        file_name = os.path.join(path, "README.md")
        with open(file_name, 'w', encoding='utf-8') as f:
            f.write('<p align="center"><img width="300" src="https://github.com/congve1/leetcode/blob/master/pictures/site-logo.png?raw=true"></p>\n')
            f.write('\n| # | Title | Solution | Difficulty |\n|:--:|:-----:|:---------:|:----:|\n')
            cursor = self.conn.cursor()
            cursor.execute("SELECT * FROM question order by frontend_id asc")
            for row in cursor:
                question_detail = {
                    'id': row[0],
                    'frontedId': row[1],
                ...
                if not self.filter_question(question_detail, filters):
                    continue
                id_slug = "{:0>3d}-{}".format(question_detail['frontedId'],
                                              question_detail['slug'])
                f.write('|')
                contents = [str(question_detail['frontedId']),
                "["+ question_detail['titlecn']+"](./"+id_slug+"/README.md)",
                "[Python3](./"+id_slug+"/"+id_slug+".py)",
                question_detail['difficulty']
                ]
                f.write("|".join(contents))
                f.write("|\n")
            cursor.close()
            print("Generate README SUCCESS")
...
if __name__=='__main__':
    ...
    test.generate_questions_code_snippets(args.output, filters)

    if argsDict.get('code'):
        test.generate_questions_submission(args.output, filters)
    test.generate_markdown(args.output, filters)
